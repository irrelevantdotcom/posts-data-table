...
/**
MODIFIED ROB O'DONNELL AUGUST 2018, ADD post_type, tax, term [taxonomy] ARGUMENTS AND images COLUMN
...
class Posts_Data_Table_Simple {
	...
	public function __construct() {
		if ( empty( self::$column_defaults ) ) {
			...
			self::$column_defaults = array(
				...
				'taxonomy'	 => array(
					'heading'	 => __( 'Taxonomy', 'posts-data-table' ),
					'priority'	 => 7,
					'width'		 => ''
				),
				'tags'		 => array(
		...
		self::$allowed_columns = array_merge( self::$allowed_columns, array_keys( self::$column_defaults ) );
	...
	public function get_table( $args ) {
		...
		if ( !empty( $args['term'] ) ) {
//echo"rob>";print_r($args);
			if ( !is_numeric( $args['term'] ) ) {	// not numeric
			    if (($term_id = get_term_by( 'slug', $args['term'], $args['tax'])) !== false) {
...
//		$all_posts = get_posts( apply_filters( 'posts_data_table_query_args', $post_args, $args ) );
		$all_posts = get_posts( array_merge( $args, $post_args ) );
...
//			'exclude'     => get_post_thumbnail_id(),
		...
		// get image from either first attachment or featured image if has none.
		if ( $attachments ) {
		...
		$thumb_url = $thumb_url_array[0];
			...
			$post_data_trans = apply_filters( 'posts_data_table_row_data_format', array(
				...
				'{taxonomy}'	=> implode( ', ', wp_get_post_terms( $_post->ID, $args['tax'], array('fields' => 'names'))),
		...
		$paging_attr = 'false';
		if ( ( $args['rows_per_page'] > 1 ) && ( $args['rows_per_page'] < count( $all_posts ) ) ) {
			$paging_attr = 'true';
		}
